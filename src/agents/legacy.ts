/**
 * 旧确定性管线(D2-B legacy 模式)
 *
 * LangChain 0.3 时代的 queryRouter → retrievalStage → prompt 拼接 → LLM 生成
 * 管线。AGENT_MODE=classic 时由 index.ts 委托调用,DeepAgent.js 迁移期间
 * 作为回退方案保留。新代码应使用 index.ts 的 deep 模式。
 */
 ...
 */
export async function legacyAgenticRag(
  input: string,
  chatHistory?: string,
